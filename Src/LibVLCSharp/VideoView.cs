namespace LibVLCSharp.Platforms.UWP
{
    ...
    /// <summary>
    /// The VideoView implementation for  U W P applications
    /// </summary>
    public class VideoView : UserControl, IVideoView
    {
        ...
        /// Calling this property will throw an <see cref="InvalidOperationException"/> 
        ...
        public string[] SwapChainOptions
        {
            get
            {
                if (!_loaded)
                {
                    ...
                    Debug.WriteLine("You must wait for the VideoView to be loaded before calling GetSwapChainOptions()");
                }
...
//#endif
                ...
                foreach (var adapter in dxgiFactory.Adapters)
                {
                    try
                    {
                        _d3D11Device = new SharpDX.Direct3D11.Device(adapter);
                        //adapter, deviceCreationFlags);
                        break;
                    }
                    catch //(SharpDXException)
                    {
                ...
                {
                    Debug.WriteLine("[ex] VideoView: Could not create Direct3D11 device " +
                        ": No compatible adapter found.");
                    throw new VLCException("Could not create Direct3D11 device " +
                        ": No compatible adapter found.");
                }
                ...
                _swapChain = new SharpDX.DXGI.SwapChain1(dxgiFactory, 
                    _d3D11Device, ref swapChainDescription);
        ...
        readonly Guid SWAPCHAIN_WIDTH = new Guid(
            0xf1b59347, 0x1643, 0x411a, 0xad, 0x6b, 0xc7, 0x80, 0x17, 0x7a, 0x6, 0xb6);

        readonly Guid SWAPCHAIN_HEIGHT = new Guid(
            0x6ea976a0, 0x9d60, 0x4bb7, 0xa5, 0xa9, 0x7d, 0xd1, 0x18, 0x7f, 0xc9, 0xbd);

        /// <summary>
        /// Associates width/height private data into the SwapChain, so that VLC
        /// knows at which size to render its video.
        /// </summary>
        void UpdateSize()
        {
        ...
        void UpdateScale()
        {
            ...
            { 
                M11 = 1.0f / _panel.CompositionScaleX, M22 = 1.0f / _panel.CompositionScaleY
            };
        }

        /// <summary>
        /// When the app is suspended, U W P apps should call Trim 
        /// so that the DirectX data is cleaned.
        /// </summary>
        void Trim()
        {
            _device3?.Trim();
