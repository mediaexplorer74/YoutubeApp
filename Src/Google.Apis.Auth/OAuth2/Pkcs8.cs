...
// Type: Google.Apis.Auth.OAuth2.Pkcs8
...
namespace Google.Apis.Auth.OAuth2
{
  internal class Pkcs8
  {
    ...
    {
      pkcs8PrivateKey.ThrowIfNullOrEmpty(nameof (pkcs8PrivateKey));
      pkcs8PrivateKey = pkcs8PrivateKey.Trim();
      object[] objArray = pkcs8PrivateKey.StartsWith("-----BEGIN PRIVATE KEY-----") && pkcs8PrivateKey.EndsWith("-----END PRIVATE KEY-----") ? (object[]) ((object[]) Pkcs8.Asn1.Decode(Convert.FromBase64String(pkcs8PrivateKey.Substring("-----BEGIN PRIVATE KEY-----".Length, pkcs8PrivateKey.Length - "-----BEGIN PRIVATE KEY-----".Length - "-----END PRIVATE KEY-----".Length))))[2] : throw new ArgumentException(string.Format("PKCS8 data must be contained within '{0}' and '{1}'.", (object) "-----BEGIN PRIVATE KEY-----", (object) "-----END PRIVATE KEY-----"), nameof (pkcs8PrivateKey));
    ...
    internal static byte[] TrimLeadingZeroes(byte[] bs, bool alignTo8Bytes = true)
    {
      ...
      if (count < bs.Length)
        Buffer.BlockCopy((Array) bs, bs.Length - count, (Array) dst, 0, count);
      else
        Buffer.BlockCopy((Array) bs, 0, (Array) dst, count - bs.Length, bs.Length);
      return dst;
    }
    ...
    internal class Asn1
    {
      ...
      internal class Decoder
      {
        ...
        private byte[] ReadBytes(int length)
        {
          ...
          Array.Copy((Array) this._bytes, this._index, (Array) destinationArray, 0, length);
          this._index += length;
