...
namespace YoutubeExplode.Internal
{
    internal static class Heuristics
    {
        ...
        public static VideoEncoding VideoEncodingFromString(string str)
        {
            ...
            Debug.WriteLine("[!] " + $"Unrecognized video encoding [{str}].", nameof(str));
            return default;
        }
        ...
        public static Container ContainerFromString(string str)
        {
            ...
            Debug.WriteLine("[!] " + $"Unrecognized container [{str}].", nameof(str));
            return default;
        }
        ...
        {
            bool r = ContainerToFileExtensionMap.TryGetValue(container, out var extension);
            ...
            Debug.WriteLine("[!]" + $"Unrecognized container [{container}].", nameof(container));
        ...
        {
            var r = ItagToVideoQualityMap.TryGetValue(itag, out var quality);

            if (r)
                return quality;
            //    ? quality
            //    : throw new ArgumentException($"Unrecognized itag [{itag}].", nameof(itag));
            Debug.WriteLine("[!] " + $"Unrecognized itag [{itag}].", nameof(itag));
        ...
        public static VideoQuality VideoQualityFromLabel(string label)
        {
            ...
            Debug.WriteLine("[!] " + $"Unrecognized video quality label [{label}].", nameof(label));
            return default;
        }
        ...
        {
            bool r = VideoQualityToResolutionMap.TryGetValue(quality, out var resolution);

            if (r)
                return resolution;
            //throw new ArgumentException($"Unrecognized video quality [{quality}].", nameof(quality));

            Debug.WriteLine("[i] " + $"Unrecognized video quality [{quality}].", nameof(quality));
            return default;
        }
    }
}